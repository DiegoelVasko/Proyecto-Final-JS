...
function ordenarProductos() {
  ...
  if (orden === "menorPrecio") {
    ...
    productos.sort((a, b) => (a.modelo < b.modelo ? -1 : 1));
  } else if (orden === "za") {
    productos.sort((a, b) => (a.modelo > b.modelo ? -1 : 1));
  }
  mostrarProductos();
...
const calcularTotal = () => {
  ...
  total.innerHTML = `U$S ${totalCompra}`;
  return totalCompra;
};
...
comprarCarro.addEventListener("click", async () => {
  const criptoYa = "https://criptoya.com/api/dolar";
  fetch(criptoYa)
    .then((response) => response.json())
    .then(async ({ blue, solidario }) => {
      await Swal.fire({
        ...
        inputValidator: (value) => {
          return new Promise((resolve) => {
            if (value === "efectivo") {
              const efectivo = Math.round(blue * 0.8 * calcularTotal());
              resolve(`El total en efectivo es: $ ${efectivo}`);
            } else if (value === "unPago") {
              const unPago = Math.round(solidario * calcularTotal());
              resolve(`El total es: $ ${unPago}`);
            } else {
              const cuotas = Math.round(solidario * 1.3 * calcularTotal());
              const cadaCuota = Math.round(cuotas / 3);
              resolve(`El total es: $ ${cuotas} en 3 cuotas de $ ${cadaCuota}`);
            }
          });
        },
      });
